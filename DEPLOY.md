...
только пользователей `demo.*@example.com`, группы `ДЕМО-*`, курсы `[ДЕМО] *` и
связанные с ними тестовые данные. При повторном запуске также удаляются старые
англоязычные демо-записи с префиксами `DEMO-*` и `[DEMO] *`.
