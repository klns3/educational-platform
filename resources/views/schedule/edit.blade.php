@php
    ...
    $labels = [
        'heading' => 'Редактирование события',
        'subtitle' => 'Измените параметры расписания',
        'title' => 'Название',
    ...
    <div class="min-h-screen bg-[#f6f8fc] text-slate-950 transition-colors duration-300 dark:bg-[#07111f] dark:text-white">
        <div class="relative overflow-hidden">
            ...
            <div class="relative mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
                ...
                <form method="POST"
                      action="{{ route('schedule.update', $event) }}"
                    ...
                    <div class="grid gap-6 p-6">
                                ...
                                <div>
                                    ...
                                    <select id="teacher_id"
                                        ...
                                        @foreach($teachers as $teacher)
                                            <option value="{{ $teacher->id }}" @selected((int) old('teacher_id', $event->teacher_id) === $teacher->id)>
                                                {{ $teacher->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div>
                                ...
                                <select id="class_group_id"
                                    ...
                                    @foreach($groups as $group)
                                        <option value="{{ $group->id }}" @selected((int) old('class_group_id', $event->class_group_id) === $group->id)>
                                            {{ $group->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="course_id"
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" @selected((int) old('course_id', $event->course_id) === $course->id)>
                                            {{ $course->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <textarea id="description"
                                      name="description"
                                      rows="5"
                                      class="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-950 outline-none transition focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-500/10 dark:border-white/10 dark:bg-white/[0.04] dark:text-white">{{ old('description', $event->description) }}</textarea>
                        </div>
                    </div>
